...
- [x] **Deployment prep** — `VITE_API_URL` env var introduced; all frontend fetch calls use `import.meta.env.VITE_API_URL || 'http://localhost:8000'`. `.env.development` sets `localhost:8000`; `.env.production` has `PLACEHOLDER.onrender.com` (replace with real URL before deploy). `vercel.json` with SPA rewrite rule added. Backend CORS opened to `allow_origins=["*"]` for deployment (tighten to exact Vercel URL post-deploy; note: `allow_credentials` removed as it's incompatible with `*`). `requirements.txt` pinned to exact versions. Both env files added to `.gitignore`.
